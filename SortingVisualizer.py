...
def draw(draw_info, algo_name, ascending):
	...
	title = draw_info.LARGE_FONT.render(f"{algo_name} {'' if ascending else ''}", 1, draw_info.GREEN)
	draw_info.window.blit(title, (draw_info.width/2 - title.get_width()/2 , 5))

	controls = draw_info.FONT.render("R - Reset | SPACE - Start Sorting", 1, draw_info.BLACK)
	draw_info.window.blit(controls, (draw_info.width/2 - controls.get_width()/2 , 45))

	sorting = draw_info.FONT.render("I - Insertion Sort | G - Gnome Sort", 1, draw_info.BLACK)
